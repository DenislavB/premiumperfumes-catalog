export type Prize = {
  label: string;     // Bulgarian label (stored)
  labelEn: string;
  short: string;     // short label shown on the wheel
  type: "percent" | "fixed" | "freebie";
  value: number;     // percent or EUR; 0 for freebie
  weight: number;    // relative probability
  color: string;     // wheel segment color
};

// Order = wheel segment order (index 0 at the top, going clockwise)
export const PRIZES: Prize[] = [
  { label: "5% отстъпка",       labelEn: "5% off",        short: "5%",       type: "percent", value: 5,  weight: 26, color: "#C9A84C" },
  { label: "Безплатна отливка", labelEn: "Free decant",   short: "Отливка",  type: "freebie", value: 0,  weight: 16, color: "#1A1612" },
  { label: "10% отстъпка",      labelEn: "10% off",       short: "10%",      type: "percent", value: 10, weight: 22, color: "#9A7A2E" },
  { label: "−5 € отстъпка",     labelEn: "€5 off",        short: "−5€",      type: "fixed",   value: 5,  weight: 18, color: "#1A1612" },
  { label: "Безплатна отливка", labelEn: "Free decant",   short: "Отливка",  type: "freebie", value: 0,  weight: 14, color: "#E8D5A3" },
  { label: "Безплатна доставка", labelEn: "Free shipping", short: "Доставка", type: "freebie", value: 0,  weight: 4,  color: "#1A1612" },
];

// Weighted random prize index
export function pickPrizeIndex(): number {
  const total = PRIZES.reduce((s, p) => s + p.weight, 0);
  let r = Math.random() * total;
  for (let i = 0; i < PRIZES.length; i++) {
    r -= PRIZES[i].weight;
    if (r <= 0) return i;
  }
  return 0;
}
